...
- (void)configureConstraintsForSubviews
{
    ...
    if (self.calendarManager.calendarAppearance.isWeekMode) {
        ...
        totalHeight =self.frame.size.height+5;
    }
    
    CGFloat height = (totalHeight-self.calendarManager.calendarAppearance.calenderHeaderHeight) / (weeksToDisplay-1);
    
    for(int i = 0; i < self.subviews.count; ++i){
        UIView *view = self.subviews[i];
        if (i==0) {
            view.frame = CGRectMake(0, y, width, self.calendarManager.calendarAppearance.calenderHeaderHeight);
            y = CGRectGetMaxY(view.frame);
        }else{
            view.frame = CGRectMake(0, y, width, height);
            y = CGRectGetMaxY(view.frame)-1;
        }
