...
- (void)commonInit
{
    ...
    {
        circleView = [[JTCircleView alloc]init];
...
- (void)configureConstraintsForSubviews
{
    ...
    dotView.center = CGPointMake(self.frame.size.width / 2., (self.frame.size.height / 3.*2.6));
    //dotView.layer.cornerRadius = sizeDot / 2.;//此句 为加圆角，无用.
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected){//方格被选中
        ...
        if(!self.isOtherMonth){                 //本月的某个方格 被选中.
            
            circleView.image = [self.calendarManager.calendarAppearance dayCircleSelectedImage];
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImageSelected];
        
        }else{                                  // 其他月 的某个方格 被选中.
           
            
            circleView.image = [self.calendarManager.calendarAppearance dayCircleSelectedOtherMonthImage];
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImageSelectedOtherMonth];
    ...
    }else if([self isToday]){ //方格未被选中，但确实 “当天”
        
        if(!self.isOtherMonth){                //当月的当前

            circleView.image = [self.calendarManager.calendarAppearance dayCircleTodayImage];
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImageToday];
            
        }
        else{                                   //其他月的当前
            
            //存在背景图片时，背景颜色设置将失效.
            circleView.image = [self.calendarManager.calendarAppearance dayCircleTodayOtherMonthImage];
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImageTodayOtherMonth];
        }
    }
    else{//未被选中 也不是当天
        if(!self.isOtherMonth){
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImage];
        }
        else{
            ...
            dotView.image = [self.calendarManager.calendarAppearance dayDotImageOtherMonth];
        }
...
- (void)reloadAppearance
{
    ...
    //设置方格 背景图片
    if (self.calendarManager.calendarAppearance.dayBackgroundImage!=nil) {
         backgroundView.image = self.calendarManager.calendarAppearance.dayBackgroundImage;
    }
